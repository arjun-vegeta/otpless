...
1. Before you proceed with integrating Google and Facebook login using the Google SDK and Facebook SDK, make sure to complete the integration steps from the [OTPless Android Headless SDK Documentation](https://otpless.com/docs/frontend-sdks/app-sdks/android/new/headless/intro).
...
<Tabs>
  <Tab title="Google">
    ...
    <CodeGroup>
      ...
      final HeadlessRequest googleSdkRequest = new HeadlessRequest();
      googleSdkRequest.setChannelType(HeadlessChannelType.GOOGLE_SDK);
      otplessView.startHeadless(googleSdkRequest, this::onHeadlessCallback);
      ```

      ```kotlin Kotlin theme={null}
      val googleSdkRequest = HeadlessRequest()
      googleSdkRequest.setChannelType(HeadlessChannelType.GOOGLE_SDK)
  ...
  <Tab title="Facebook">
    ...
    <CodeGroup>
      ```java Java theme={null}
      final HeadlessRequest facebookSdkRequest = new HeadlessRequest();
      facebookSdkRequest.setChannelType(HeadlessChannelType.FACEBOOK_SDK);
      otplessView.startHeadless(facebookSdkRequest, this::onHeadlessCallback);
      ```

      ```kotlin Kotlin theme={null}
      val facebookSdkRequest = HeadlessRequest()
      facebookSdkRequest.setChannelType(HeadlessChannelType.FACEBOOK_SDK)
      otplessView.startHeadless(request, this::onHeadlessCallback)
      ```
    </CodeGroup>
